...
需要新增/修改 ViewModel 属性时，**禁止手改生成文件**，须走严格三阶段（顺序门，不可跳过/乱序）：
...
- **Phase 2（硬门）**：用 unity-cli 生成 ViewModel。
  ...
  - **生成成功前禁止进入 Phase 3**（C# 常量与 Lua 映射此前不存在，提前写会编译错误 / Lua 静默失败）。
...
> 若生成能力不可用 / 用户拒绝退出 PlayMode → 停在 Phase 2，在 `HUMAN_REVIEW.md` 标注
> 「请在 Unity 中手动退出 PlayMode、刷新、重新生成 ViewModel」，**不进入 Phase 3**，不在管线中途停顿。
...
- ❌ **手改自动生成文件**：`*_viewmodel.lua`、`*ViewModel.cs`、`AtomViewModelFactory.cs`、`ui_viewmodel_define.lua`。改属性只改 ViewModelDes 再重新生成。
  - 例外：`*_data.lua`（配表导出产物）**不是** MVVM 生成文件，`gui-config` 故意镜像写入（模拟导表），见 `verification-gates.md`。
