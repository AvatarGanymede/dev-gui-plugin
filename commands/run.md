...
3. **gui-plan 入口参数必须先问齐；后续缺信息才留 TODO**：
   - `panelId` 和功能描述是 gui-plan 的硬入口参数。进入 plan mode、读取仓库或执行其他动作前必须先检查；
     用户缺少任一项时，用 `AskUserQuestion` 一次性询问所有缺失项并等待回答，**不得推断或用占位值继续**。
   - 两项齐全后，若 Prefab 路径 / ViewModel 类型等其他信息缺失或无法从
...
### 0. 检查入口参数 + 硬断言 session_id（纯只读；先不落地任何文件）

**最先检查用户输入**：先从本次需求中确认用户是否已明确提供 `panelId` 和功能/需求描述。
缺少任一项时，立即用 `AskUserQuestion` 一次性询问所有缺失项（只缺一项则只问该项），并等待用户回答；
此检查完成前不得进入 plan mode、读取仓库、执行 `runid` 或使用合理占位值。把补充回答与原始输入合并为
本次 run 的有效输入，再继续下方步骤。
...
- 由编排者从已通过前置检查的有效输入中解析出 `panelId` 作为 `PID`。
...
1. **加载 gui-plan 并进入 plan mode**：加载并执行 `${CLAUDE_PLUGIN_ROOT}/skills/gui-plan/SKILL.md`，
   先执行其中的入口参数检查。只有 panelId 和需求描述齐全后，才可调用 `EnterPlanMode`（已在则跳过）。
   plan mode 内**只做只读**：`Read`
   `code/LuaScripts/client/data/ui_data.lua` 定位 Prefab/脚本/ViewModel；用 SessionStart 已注入
   上下文的两库 query_pack 坑点。
2. **查代码自答，无法确定必须提问**：入口两项之外仍有需求缺口时，先穷尽代码仓库（`ui_data.lua`、
   同目录现有 panel/View/ViewModel、`shared-references`、已注入的 query_pack），能推断的一律自己定；
