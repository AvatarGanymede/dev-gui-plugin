...
4. **优先查代码自答，尽量不提问**：需求有缺口时**先穷尽代码仓库**——读 `ui_data.lua`、同目录现有
   panel/View/ViewModel、`shared-references`、已注入的 query_pack。凡能从仓库推断的（panelId、Prefab
   路径、ViewModel 字段惯例等）一律自己定，**不问用户**；能占位 + 标 `**[TODO: 待人工确认]**` 推进的，
   优先占位而非提问。**只有**查遍仓库仍无法确定、且该缺口会实质影响需求正确性时，才用 `AskUserQuestion`
   **一次性**问完所有此类问题（不逐条追问）。此阶段无 autorun 哨兵，可自由停顿，但停顿成本高——把提问
   压到最少。
...
> ViewModel 设计缺信息（如属性类型未定）→ 标 `**[TODO: 待人工确认]**`，不要在 gui-draft 现推。
