...
4. **加载知识库（两库都读）**：
   - **私有库**：若 `${CLAUDE_PLUGIN_DATA}/gui-knowledge/` 不存在 → 先初始化：
     ```bash
     ...
     读 `${CLAUDE_PLUGIN_DATA}/gui-knowledge/query_pack.md`。
   - **公共库**：`${CLAUDE_PROJECT_DIR}/.claude/dev-gui-knowledge/query_pack.md` —— **存在则读、不存在静默跳过**
     （公共库**不在此自动 init**，仅 gui-learn 显式写入时创建）。
   - 合并两库，提取与本需求相关的历史坑点/组件技巧；两库矛盾以公共库为准。
     （SessionStart hook 已尝试预注入两库，此处兜底确认。）
5. **初始化运行状态**：
   ```bash
